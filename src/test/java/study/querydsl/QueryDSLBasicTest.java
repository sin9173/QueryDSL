...
public class QueryDSLBasicTest {
    ...
    public void search() {
        ...
        //member.startWith("member") -> member%
        Member findMember = queryFactory
                .selectFrom(member)
...
//        List<Member> content = results.getResults();
    ...
    /*
        회원과 팀을 조회하면서 팀이름이 teamA인 팀만 조인, 회원은 모두 조인
    ...
    public void findDtoBySetter() {
        List<MemberDto> result = queryFactory
                .select(
                        Projections.bean( //Setter 를 통해서 값이 들어감
                                MemberDto.class,
                                member.username,
                                member.age))
                .from(member)
    ...
    public void findDtoByConstructor() {
        List<UserDto> result = queryFactory
                .select(
                        Projections.constructor( //생성자를 통해 값을 넣어줌
                                UserDto.class,
                                member.username,
                                member.age))
                .from(member)
    ...
    public void findUserDto() {
        ...
        List<UserDto> result = queryFactory
                .select(
                        Projections.fields( //필드에 바로 들어감
                                UserDto.class,
                                member.username.as("name"), //필드가 일치하지 않을 경우 null 이 들어감
    ...
    private List<Member> searchMember2(String usernameCond, Integer ageCond) {
        return queryFactory
                .selectFrom(member)
                .where(usernameEq(usernameCond), ageEq(ageCond)) //null 조건이 들어갈 경우 무시됨
                .fetch();
    }
    ...
    public void bulkUpdate() {
        //벌크 연산시 영속성 컨텍스트를 거치지 않고 쿼리를 보냄
